...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    echo "<script>alert('Không tìm thấy thông tin!'); window.history.back();</script>";
    exit();
}
...
if ($user_res->num_rows == 0) {
    echo "<div class='container py-5 text-center'><h3>Tài khoản không tồn tại.</h3></div>";
...
if (!empty($tutor['dob']) && $tutor['dob'] != '0000-00-00') {
    ...
    $now = new DateTime();
    // Tính tuổi bằng cách lấy hiệu số năm
    ...
    <div class="col-lg-8">
      ...
      <div class="card card-detail mb-4">
        <div class="d-flex flex-column flex-md-row gap-4 align-items-center align-items-md-start text-center text-md-start">
            ...
            <?php else: ?>
                <div class="avatar-circle" style="width: 100px; height: 100px; font-size: 40px; margin: 0;">
                    <?= $initials ?>
                </div>
            <?php endif; ?>
            ...
            <div class="flex-grow-1">
                ...
                <div class="text-warning mb-3">
                    ...
                    <i class="bi bi-star-fill"></i>
                    <i class="bi bi-star-fill"></i>
                    <span class="text-muted small ms-1">(Tài khoản uy tín)</span>
                </div>
                
                <div class="d-flex gap-3 justify-content-center justify-content-md-start">
                    <div class="bg-light px-4 py-2 rounded border">
                        <strong class="d-block text-brand fs-5"><?= $count_all ?></strong> 
                        <small class="text-muted">Tổng lớp</small>
                    </div>
                    <div class="bg-light px-4 py-2 rounded border">
                        <strong class="d-block text-brand fs-5"><?= $count_active ?></strong> 
            ...
            <?php if($is_owner): ?>
            <div>
                <a href="personal_profile.php" class="btn btn-outline-primary btn-sm">
      ...
      <div class="card card-detail">
        ...
        <?php else: ?>
            <div class="text-center text-muted py-4">
                Hiện tại chưa có lớp nào đang mở.
            </div>
        <?php endif; ?>
      </div>
    </div>
      ...
      <div class="card card-detail">
        ...
        <ul class="list-unstyled m-0">
            <li class="info-list-item">
                ...
                <span class="fw-medium text-end ps-2 text-dark"><?= htmlspecialchars($tutor['phone'] ?? 'Chưa cập nhật') ?></span>
            </li>
            <li class="info-list-item">
                <span class="text-muted">Email</span>
                <span class="fw-medium text-end ps-2 text-dark"><?= htmlspecialchars($tutor['email']) ?></span>
            </li>
            
            <li class="info-list-item">
                <span class="text-muted">Ngày sinh</span>
                <span class="fw-medium text-end ps-2 text-dark"><?= $dob_display ?></span>
            </li>
            <li class="info-list-item">
                <span class="text-muted">Tuổi</span>
                <span class="fw-medium text-end ps-2 text-dark"><?= $age_display ?></span>
            </li>

            <li class="info-list-item">
                <span class="text-muted">Chuyên ngành</span>
                <span class="fw-medium text-end ps-2"><?= htmlspecialchars($tutor['major'] ?? 'Chưa cập nhật') ?></span>
            </li>
            <li class="info-list-item">
                <span class="text-muted">Trình độ</span>
                <span class="fw-medium text-end ps-2"><?= htmlspecialchars($tutor['degree'] ?? 'Chưa cập nhật') ?></span>
            </li>
            <li class="info-list-item">
                <span class="text-muted">Kinh nghiệm</span>
                <span class="fw-medium text-end ps-2"><?= htmlspecialchars($tutor['experience'] ?? 'Chưa cập nhật') ?></span>
            </li>
            <li class="info-list-item">
                <span class="text-muted">Khu vực</span>
                <span class="fw-medium text-end ps-2"><?= htmlspecialchars($tutor['address'] ?? 'Chưa cập nhật') ?></span>
            </li>
            <li class="info-list-item border-0">
                <span class="text-muted">Tham gia từ</span>
                <span class="fw-medium"><?= date('m/Y', strtotime($tutor['created_at'])) ?></span>
            </li>
        </ul>
      </div>
    </div>
